  var cordova = window.cordova || require('cordova');
  ...
  function _init() {
      ...
      msgDIV.push('<span>' + Toast.message + '</span>');
      msgDIV.push('</div>');
      msgEntity = $(msgDIV.join('')).appendTo(Toast.context);
      //设置消息样式  
      var left = Toast.left == null ? Toast.context.width() / 2 - msgEntity.find('span').width() / 2 : Toast.left;
      var top = Toast.top == null ? '20px' : Toast.top;
      /*   msgEntity.css({
             margin: '10px'
         });*/
      msgEntity.hide();
  ...
  function _show() {
      msgEntity.fadeIn(Toast.time / 100);
      msgEntity.fadeOut(Toast.time);
  }

  function Toast(config) {
      Toast.context = config.context == null ? $('body') : config.context; //上下文  
      Toast.message = config.message; //显示内容  
      Toast.time = config.time == null ? 5000 : config.time; //持续时间  
      Toast.left = config.left; //距容器左边的距离  
      Toast.top = config.top; //距容器上方的距离  
      //  Toast.init();
  }
